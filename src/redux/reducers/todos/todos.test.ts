...
import {deleteTodoListAC, addTodoListAC, changeTodoListTitleAC, changeTodoListFilterAC, setTodosAC} from './todos';
...
test('todoLists should be set to State', () => { 
    const endState = todoReducer([], setTodosAC(startState))
    expect(endState.length).toBe(2);
})